...
def save_model_and_result(dt_string, epoch, train_loss, test_loss,
                          best_iou, nIoU, recall, precision,
                          save_mIoU_dir, save_other_metric_dir):

    # 保存 IoU 和 nIoU（原先的 mIoU.log）
    with open(save_mIoU_dir, 'a') as f:
        f.write('{} - {:04d}:\t - train_loss: {:04f}:\t - test_loss: {:04f}:\t mIoU {:.4f}\t nIoU {:.4f}\n'
                .format(dt_string, epoch, train_loss, test_loss, best_iou, nIoU))

    # 保存 recall 和 precision（other_metric.log）
...
# def save_model(mean_IOU, nIoU, best_iou, save_dir, save_prefix,
#                train_loss, test_loss, recall, precision, epoch, net, pd, fa):

#     if mean_IOU > best_iou:
#         save_mIoU_dir = f'result/{save_dir}/{save_prefix}_best_IoU_IoU.log'
#         save_other_metric_dir = f'result/{save_dir}/{save_prefix}_best_IoU_other_metric.log'
...
#         # --- 保存 IoU 和 nIoU ---
#         with open(save_mIoU_dir, "a") as f:
#             f.write(f"{dt_string} | Epoch {epoch} | IoU={mean_IOU:.4f} | nIoU={nIoU:.4f}\n")

#         # --- 保存额外指标 ---
#         with open(save_other_metric_dir, "a") as f:
#             f.write(f"{dt_string} | Epoch {epoch}\n")
#             f.write(f"TrainLoss={train_loss:.4f} | TestLoss={test_loss:.4f}\n")
#             f.write(f"PD={pd:.4f} | FA={fa:.6f}\n")
...
#            filename=f'mIoU_{save_prefix}_epoch.pth.tar')
def save_model(mean_IOU, nIoU, best_iou, save_dir, save_prefix,
               train_loss, test_loss, recall, precision, epoch, net, pd, fa):
    ...
    if mean_IOU > best_iou:
        ...
        # 保存日志
        with open(f'result/{save_dir}/{save_prefix}_best_metrics.log', "a") as f:
            f.write('{} - Epoch {:04d}:\tIoU={:.4f}, nIoU={:.4f}, PD={:.4f}, FA={:.6f}, TestLoss={:.4f}\n'
                    .format(dt_string, epoch, mean_IOU, nIoU, pd, fa, test_loss))

        # 保存详细指标
        with open(f'result/{save_dir}/{save_prefix}_best_other_metric.log', "a") as f:
            f.write(dt_string + ' - ' + str(epoch) + '\n')

            f.write('Recall-----: ')
            for x in recall:
                f.write('   ' + str(round(x, 8)))
            f.write('\n')
        ...
        # 保存 checkpoint — 已加入 PD 和 FA，并将 mean_IOU 改名为 IoU
        save_ckpt({
            'epoch': epoch,
            'state_dict': net.state_dict(),
            'IoU': mean_IOU,     # 👈 用 IoU 代替 mean_IOU
            'nIoU': nIoU,
            'PD': pd,            # 👈 新增
            'FA': fa,            # 👈 新增
            'TestLoss': test_loss,
        ...
        save_path=f'result/{save_dir}',
        filename=f'best_model_{save_prefix}.pth.tar')
...
def save_result_for_test(base_dir, model_name, epoch, iou, nIoU, pd, fa, recall, precision):
    # 确保value_result目录存在
    value_result_path = os.path.join(base_dir, 'value_result')
    os.makedirs(value_result_path, exist_ok=True)
    
    now = datetime.now()
    dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
    
    # 保存IoU结果 (保持原始格式，但同时包含nIoU)
    iou_file_path = os.path.join(value_result_path, f'{model_name}_best_IoU.log')
    with open(iou_file_path, 'a') as f:
        f.write(f'{dt_string} - {epoch:04d}:\n')
        f.write(f'{"IoU":<10}: {iou:.4f}\n')
        f.write(f'{"nIoU":<10}: {nIoU:.4f}\n')
        f.write(f'{"PD":<10}: {pd:.4f}\n')
        f.write(f'{"FA":<10}: {fa:.8f}\n')
        f.write('-' * 40 + '\n')
    
    # 保存其他指标
    other_metric_path = os.path.join(value_result_path, f'{model_name}_best_other_metric.log')
    with open(other_metric_path, 'a') as f:
        f.write(f'{dt_string}-{epoch}\n')
        f.write('Recall-----:')
        for i in range(len(recall)):
            f.write(f'   {recall[i]:.8f}   ')
        f.write('\n')

        f.write('Precision--:')
        for i in range(len(precision)):
            f.write(f'   {precision[i]:.8f}   ')
        f.write('\n')
        
        # 新增指标
        f.write(f'nIoU-------: {nIoU:.4f}\n')
        f.write(f'PD---------: {pd:.4f}\n')
        f.write(f'FA---------: {fa:.8f}\n')
        f.write('-' * 50 + '\n')
